...
class DeliveryReceiptService extends DefaultService<DeliveryReceiptInterface> {
	...
	async create(payload: DeliveryReceiptInterface) {
		...
		shipments.forEach(shipment => {
			if(!new DeliveryReceiptFlow(shipment.status).isValidReceipt(populatedDeliveryReceipt)) throw badData(getErrorMessage(shipment.code));
		});
	...
	async confirm(id: ID, authUser: AuthUser) {
		...
		if(!(authUser.user as EmployeeInterface)?.isAdmin && authUser.user._id !== deliveryReceipt.recipient._id) throw forbidden("يمكن تاكيد الايصال من قبل المستلم فقط");
		...
		shipments.forEach(shipment => {
			if(!new DeliveryReceiptFlow(shipment.status).isValidReceipt(deliveryReceipt)) throw badData(getErrorMessage(shipment.code));
		});
